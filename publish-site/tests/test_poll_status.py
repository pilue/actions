...
class TestFetchStatus(unittest.TestCase):
    ...
    def test_returns_parsed_json(self):
        ...
        with patch("urllib.request.urlopen", return_value=self._mock_response(body)):
            result = poll_status.fetch_status("https://example.com/status", "secret")
...
class TestRunPollLoop(unittest.TestCase):
    def _make_fetch(self, responses):
        """Return a fetch_status mock that yields responses in sequence."""
        it = iter(responses)
    ...
    def test_timeout(self):
        ...
        with patch.object(
            poll_status, "fetch_status", return_value={"status": "in_progress"}
        ):
            success, msg = poll_status.run_poll_loop(
                "example.pilue.co.uk",
                "ts",
    ...
    def test_network_errors_do_not_crash_loop(self):
        # First two fetches fail (empty dict), third succeeds
        responses = [{}, {}, {"status": "completed", "conclusion": "success"}]
        with patch.object(
            poll_status, "fetch_status", side_effect=self._make_fetch(responses)
        ):
